...
const CATEGORIES = [
  {
    key: "health",
    label: "Health",
    icons: { dumbbell: Dumbbell, heart: HeartPulse, bike: Bike, footprints: Footprints },
  },
  {
    key: "hygiene",
    label: "Hygiene",
    icons: { droplet: Droplet, sparkles: Sparkles, bath: Bath, smile: Smile },
  },
  {
    key: "creative",
    label: "Creative",
    icons: { pen: PenLine, music: Music, palette: Palette, camera: Camera },
  },
  {
    key: "mindfulness",
    label: "Mindfulness",
    icons: { moon: Moon, sun: Sun, wind: Wind, brain: Brain },
  },
  {
    key: "learning",
    label: "Learning",
    icons: { book: BookOpen, grad: GraduationCap, pencil: Pencil, lang: Languages },
  },
];
...
function MiniBars({ color, current }) {
  // A compact 4-bar indicator standing in for the full grid when a card is collapsed.
    ...
    <div className="flex items-end gap-[3px] h-[22px]">
      {heights.map((h, i) => (
        <div
          ...
          style={{
            height: h,
            background: i === 0 && current > 0 ? color.fill : "#2A2A32",
          }}
        />
      ))}
    </div>
  ...
  return (
    <div className="flex flex-col gap-3 bg-[#141417] rounded-2xl p-4">
      <div className="flex items-center justify-between">
        <h3 className="text-white text-[13px] font-semibold uppercase tracking-wide">
          {isEdit ? "Edit habit" : "New habit"}
        </h3>
        <button onClick={onCancel} className="text-[#6B6B75] hover:text-white" aria-label="Close">
          <X size={16} />
        </button>
      </div>
      ...
      <textarea
        ...
        placeholder="Notes (optional) — reminders, motivation, details"
        rows={2}
      ...
      <div className="flex gap-1.5 overflow-x-auto pb-1">
        {CATEGORIES.map((c) => (
          <button
            ...
            onClick={() => {
              setCategory(c.key);
              setIconKey(Object.keys(c.icons)[0]);
            }}
            className="shrink-0 px-2.5 py-1 rounded-full text-[11px] font-medium border"
            style={{
              borderColor: category === c.key ? "#fff" : "#2A2A32",
              background: category === c.key ? "#25252C" : "transparent",
              color: category === c.key ? "#fff" : "#8A8A94",
            }}
          >
            {c.label}
          </button>
      ...
      {isEdit && (
        <div className="flex items-center gap-2 bg-[#0F0F13] border border-[#2A2A32] rounded-lg px-3 py-2">
          ...
          <button
            type="button"
            onClick={() => onBackfill(initial.id, backfillDate)}
            className="text-[12px] font-medium text-white bg-[#25252C] px-2 py-1 rounded shrink-0"
          >
      ...
      <div className="flex items-center justify-between pt-1">
        <div>
          {isEdit && !confirmDelete && (
            <button
              ...
              className="text-[13px] text-[#8A8A94] hover:text-[#F24E4E] flex items-center gap-1"
            >
          ...
          {isEdit && confirmDelete && (
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="text-[12px] font-medium text-white bg-[#F24E4E] px-2 py-1 rounded"
              >
                Yes
              </button>
              <button
                ...
                className="text-[12px] text-[#8A8A94]"
              >
                Cancel
              </button>
            </div>
        ...
        <div className="flex gap-2">
          <button type="button" onClick={onCancel} className="px-3 py-1.5 text-sm text-[#9494A0] hover:text-white">
            Cancel
          </button>
          <button
            type="button"
            onClick={() => {
              if (!name.trim()) return;
              onSave({ name: name.trim(), subtitle: subtitle.trim(), notes: notes.trim(), category, icon: iconKey, color: colorName });
            }}
...
function HabitCard({ habit, onToggleToday, onOpenEdit, collapsed, onToggleCollapse }) {
  ...
  return (
    <div className="relative bg-[#141417] rounded-2xl p-4 overflow-hidden">
      <div className={`flex items-center justify-between ${collapsed ? "" : "mb-3"}`}>
        <div className="flex items-center gap-2.5 min-w-0">
          ...
          <button
            ...
            className="w-6 h-6 rounded-full bg-[#25252C] flex items-center justify-center shrink-0"
      ...
      {!collapsed && (
        <>
          ...
          {habit.notes && (
            <p className="text-[11px] text-[#6B6B75] mt-2 italic line-clamp-2">{habit.notes}</p>
          )}
          <div className="flex items-center justify-between mt-3">
            ...
            <button
              ...
              className="px-3 py-1 rounded-full bg-[#25252C] text-[12px] font-medium text-white"
            >
              Edit
            </button>
          </div>
...
export default function HabitTracker() {
  ...
  const backfillEntry = (id, dateKey) => {
    ...
    setHabits(next);
    // keep the edit panel open and in sync with the freshly saved habit
  ...
  const toggleToday = (id) => {
    const key = todayKey();
    const next = habits.map((h) =>
      h.id === id ? { ...h, entries: { ...h.entries, [key]: !h.entries[key] || undefined } } : h
    );
    next.forEach((h) => {
      ...
      <div className="max-w-md mx-auto px-4 pt-8 pb-28">
        <header className="mb-6">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-1 text-[13px] font-semibold text-[#B4B4C0]">
              Habits <ChevronDown size={13} />
            </div>
            <div className="flex items-center gap-1 bg-[#141417] rounded-full p-1">
              <button className="w-8 h-8 rounded-full flex items-center justify-center text-[#B4B4C0] hover:text-white">
                <LayoutGrid size={15} />
              </button>
              <button className="w-8 h-8 rounded-full flex items-center justify-center text-[#B4B4C0] hover:text-white">
                <Settings size={15} />
        ...
        <div className="flex flex-col gap-3">
          {habits.length === 0 && !showAdd && (
            <div className="text-center py-16 bg-[#141417] rounded-2xl">
              ...
              <button onClick={() => setShowAdd(true)} className="text-[13px] font-medium text-white underline underline-offset-2">
